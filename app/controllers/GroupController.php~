...
class GroupController extends \BaseController {
	...
	//	$id_students = Student::where('active_students','1')->lists('Firstname_students','id_students');
		...
		$id_students = Student::where('active_students','1')->orderBy('Lastname_students')->get();
		$id_students = $id_students->lists('StudentFullName','id_students');
	//	$id_teachers = Teacher::whereHas('user',function($q) {                             $q->where('active_users','1');                                                   })->lists('Firstname_teachers','id_teachers');
		...
		})->orderBy('Lastname_teachers')->get();
	        $id_teachers =$id_teachers->lists('TeacherFullName','id_teachers');	
		return View::make('groups.create',compact('id_grados','id_students','id_teachers','years'));
	...
	{
		$input = Input::all();
		$validation = Validator::make($input, Group::getRules(Input::get('year_groups')));
	...
	public function edit($id)
	{
		$group = Group::find($id);
			...
			if($group->year_groups < $y){
		          ...
		          Session::flash('message', 'Grupo inactivo, no es posible actualizar');
		          return Redirect::route('groups.index');
			}
		$id_grados = Grado::lists('id_grados', 'id_grados');
		$id_teachers = Teacher::whereHas('user', function($q){
	...
	public function destroy($id)
	{
		...
		if($group->year_groups <= $y){
			Session::flash('message','Grupo es inactivo/No es posible eliminar');
			return Redirect::route('groups.index');
		}
